...
function ExtractMetadata(node, depth, category, meta_name)
{
	meta_str = "";
	meta_cat = null;
	if ( (depth < 1) || (node.Name == meta_name) )
	{
	...
	if (meta_cat != null)
	{
		for (idx = 0; idx < meta_cat.Count; idx ++)
		{
			...
			if ( (meta_key.indexOf(meta_name) > -1) && (meta_key.indexOf("_RTF") < 0) )
			{
	...
	if ( (meta_str == "") && (depth < 2) )
	{
		...
		while (child)
		{
			...
			if (child == null)
				break;
			meta_str = ExtractMetadata(child, depth + 1, category, meta_name);
			if (meta_str != "")
				break;
	...
	MouseHandler.onEvent = function(event)
	{
		if (event.isMouseDown)
		{
			...
			if (node)
			{
				...
				SetDescriptionText(CtrlTitle, ExtractMetadata(node, 0, "PMIMetadata", "TITLE_BLOCK") );
				SetDescriptionText(CtrlNotes, ExtractMetadata(node, 0, "PMIMetadata", "NOTES") );
				SetDescriptionText(CtrlTolers, ExtractMetadata(node, 0, "PMIMetadata", "TOLERANCE") );
			}
			else
			{
...
{
	cr_anim_name = LocalEv3de.CurrentAnimation;
	for (anim_idx = 0; anim_idx < Scene.Animations.Count; anim_idx ++)
	{
		anim = Scene.Animations.getByIndex(anim_idx);
		if (anim.name == cr_anim_name)
			return anim_idx;
	}
	return 0;
}
...
function Play()
{
	if (!InitContext() )
		return;
	InitAnimation(FindCurrentAnimation() );
	if (AnimationObject)
	{
...
function TogglePMI()
{
	...
	LocalEv3de.ExecuteCommand("M3210");
}
...
function HideSelected() {
	...
	LocalEv3de.ExecuteCommand("Hide Selection");
}
...
function HideUnselected() {
	...
	LocalEv3de.ExecuteCommand("Hide Unselected");
}
...
function ShowAll() {
	...
	LocalEv3de.ExecuteCommand("Show All Objects");
}
...
function StepEvent(evnt)
{
	...
	if (evnt.isStepStarted)
	{
		...
		if (CtrlModelViews && window['d_mview_name'] != undefined)
		{
			var activeVp = Scene.GetActiveViewport().Name;
			for (i = 0; i < CtrlModelViews.length; i++) {
				CtrlModelViews.options[i].selected = (activeVp == CtrlModelViews.options[i].text);
			}
			SetDescriptionText(d_mview_name, activeVp);
		}
	}
